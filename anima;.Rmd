...
animals <- read_csv("animal_data.csv")
...
The chunks of code below calculate all of the peices we'll need in our assembly line. Each chunk is a station. The Fisher assumptions are built into the way we are calculating our assembly line instructions and when simulating the data below. 

```{r}
animals_sim <- animals %>%
  mutate(benchmark = mean(calm)) #universal factor
```
...
animals_sim <- animals_sim %>%
  mutate(calm_sim = benchmark 
         ...
         + interaction_effect 
         + student_effect
         + rnorm(68, 0, 0.65)) #SIMULATED universial factor
```
