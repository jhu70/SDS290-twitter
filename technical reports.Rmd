...
sum_stats <- df %>%
  group_by(type, time) %>%
  summarise(mean = mean(sum_engage),
            sd = sd(sum_engage),
            min = min(sum_engage),
            max = max(sum_engage),
            range = max - min) %>%
...
N: The distribution of error sizes should follow a normal curve. According to the histogram above, this assumption is violated since the histogram is right-skewed and we have no observation for the promotional type of post. 
